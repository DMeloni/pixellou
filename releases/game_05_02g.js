...
var deplacePlateformeDeHautEnBas = function(){
	plateformesHorizontales.forEach(function(plateformeVerticale, index){
		...
		plateformeVerticale.setPosition((plateformeVerticale.X) * 16, (plateformeVerticale.Y + plateformeVerticale.sensdeplacement)  * 16);	
		plateformeVerticale.deplacement += plateformeVerticale.sensdeplacement;
	});
}
...
var GameLoop = function(){	
	...
	if(map_modifiee[parseInt((player.y) / 16)][parseInt(player.x/16  + 1)] == "2"
	|| map_modifiee[parseInt((player.y) / 16)][parseInt(player.x/16 - 1)] == "2"
	|| map_modifiee[parseInt((player.y) / 16) + 1][parseInt(player.x/16)] == "2"
	){
		GameOver(score, 'WIN');	
	}
