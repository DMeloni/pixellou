...
var Player = function(){
	...
	that.deplacement = function(deplacementValue){
		...
		that.repositionnement();
		if(defilementPossible
		 &&((deplacementValue < 0 && (that.x < 0 - 10 * deplacementValue))// Gauche
		 || (deplacementValue > 0 && (that.x > width - 10 * deplacementValue )))){
			murs.forEach(function(mur, index){
			...
			if(defilement){
				...
				var i;
				if(deplacementValue < 0){
					for(i = nbMurs - 1 ;  i >= 0 ; i--){
						var mur = murs[i];
						map_modifiee[parseInt((mur.y) / 16)][parseInt((mur.x) / 16)] = "0";
						map_modifiee[parseInt((mur.y) / 16)][parseInt((mur.x - deplacementValue) / 16)] = "1";
						mur.setPosition(mur.x - deplacementValue, mur.y );				
					}
				}else{
					for(i = 0 ; i < nbMurs ;  i++){
						var mur = murs[i];
		...
		else{if( map_modifiee[parseInt((that.y) / 16)][parseInt((that.x + deplacementValue) / 16)] == "0"
			){
